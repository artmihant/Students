...
# Это краткое пособие по языку Python написано на корректном коде языка Python версии 3.6.9
# Многочисленные комментарии обозначаются символом #.


# Настоящий документ является вольным дополненным переводом справки по синтаксису языка программирования, доступной в оригинале по адресу http://cheat.sh/python/:learn .
# Пользуясь случаем, рекомендую этот ресурс для ознакомления с новыми языками и инструментами.


# Для удобной работы соиспользуйте c пособием любой редактор кода с подсветкой.  
# В качестве редактора кода с подсветкой я рекомендую notepad++ или sublime text.
...
# Для работы с Python вам потребуется установленный в вашей операционной системе интерпретатор языка программирования.
...
# Таким образом, вы запустите интерпретатор в интерактивном режиме. В этом режиме вы можете заниматься отладкой вашего кода, проврять синтаксис или использовать язык как программируемый калькулятор.
...
# Можно обратиться к отдельному символу.
...
# Удобнее использовать метод format -
...
# Так же есть буквы u и b , обозначающие юникодные и байтовые строки.

# None это пустой обьект
None  # => None
...
# Очень полезно для работы с обьектами.
# Объектом в питоне считается абсолютно всё, что можно поместить в переменную.
# Числа и строки тоже объекты.
# Существует важное разделение на изменяемые и неизменяемые типы объектов. 

# Любой обьект может быть задействован в булевом контексте.
# Вот что вернет False:
#    - None
#    - ноль любого числового типа (т.к., 0, 0L, 0.0, 0j)
#    - пустая последовательность (т.к., '', (), [])
#    - пустой контейнер (т.к., {}, set())
#    - экземпляры пользовательских классов в определенных условиях
#
...
# В python есть оператор вывода (обычно в консоль, где исполняется скрипт).
...
# Очень часто в студенческих задачах нужно принять из консоли число. Это делается так:
num1 = int(input("Введите целое число: "))
num2 = float(input("Введите дробное число: "))

# Как вы уже, наверное, догадались, любое имя типа: int, float, bool, str
#   может быть использовано как функция приведения типов.
...
some_other_var  # Вызовет NameError
...
# Используете списки для данных, которые могут дополняться 
...
# Можно подставить в get аргумент, который вернется, если не найдется ключ
...
# Последняя из базовых структур данных - множества (set).
...
"""
"range(number)" вернет список (точнее говоря, итератор) целых чисел от нуля до number
напечатает:
    0
    1
    2
...
# С файлами можно работать вот так (считает и напечатает текстовый файл):
...
# Но лучше применить конструкцию with - as.
# Тогда вам не нужно будет закрывать дескрипт файла явно - это будет сделано автоматически.
...
# можно передавать всю информацию, переданную в функцию, по цепочке, в другие функции
...
# Суть в том, что вы описываете сущностно связанные данные и основную логику работы с ним в виде специальной структуры, называемой классом. Затем вы создаете экземпляры класса - объекты, помещая в обьект данные, характерные для него. После этого, всё, что вам остается делать сводится к манипуляции объектами.
# Терминология ООП:
#     Класс - описание на языке программирования того, какие данные у нас есть и что с ними можно сделать. В питоне, типы и классы отличаются в основном тем, что типами называются изначально встроенные конструкции, а классами - привнесенные.
...
#     Наследование - механизм, позволяющий классам наследоваться друг-от-друга. При этом те свойства класса-наследника, которые не были переписаны явно, остаются такими же, как у класса-предка. Иногда это удобно, и позволяет сильно сократить код.
#     Интерфейс - набор методов, через которые предпологается взаимодействовать с обьектом извне.
#     Инкапсуляция - механизм, позволяющий запретить использовать некоторые методы, вызывая их у обьекта явно. Полезно, если у объекта есть продуманный интерфейс, и операции не через него могут привести к непрогнозируемому исходу.
#     Полиморфизм - возможность прописать совершенно различным классами (или типам) логику работы через одинаковые интерфейсы. В питоне реализовано, в частности, через магические методы.
...
class Human(object):
    ...
    # Обратите внимание на __ справа и слева.
    # Двойное подчеркивание означает встроеный в синтаксис языка метод (магический метод)
    # Справка по магическим методам https://habr.com/ru/post/186608/
    def __init__(self, name):
        # Первый атрибут метода класса self - сам объект
        # Вот так объекту даеются при инициализации новые артибуты
        self.name = name
        self.age = 0

    # Так же, первый атрибут метода класса self - сам объект
    def say(self, msg):
        return f"{self.name} сказал {msg}"

    # Здесть применен @декоратор. Декораторы можно ставить над описанием метода или функции, и они будут изменяеть этот метод неким образом.
    # Метод класса @classmethod передаст первым аргуметом не обьект, а сам класс.
    @classmethod
    def get_species(cls):
        return cls.species

    # Статический метод @staticmethod вызывается без передачи в него ссылки на класс или обхект
    @staticmethod
    def grunt():
        return "*хрюк*"

    # @property создает свойство обьекта. При этом происходит следующее: значение внутри self.age помещается в self._age . Методы и атрибуты, начинающиеся на _ и не заканчивающиеся _ недоступны извне (см. инкапсуляция). После этого self.age заменяется на функцию self.age(). Теперь нельзя перезаписать возраст вот так: some_human.age = 100 , свойство age стало только для чтения.
    @property
    def age(self):
        return self._age

    # Теперь переписать возраст some_human.age = 100 можно, и при этом напечатается сообщение.
    @age.setter
    def age(self, age):
        print(f"{self.name} now {age} age old!")
        self._age = age

    # Обработчик удаления свойств
    @age.deleter
    def age(self):
        del self._age

# Инициализация класса. Мы создали "человека" с именем "Ivan"
ivan = Human(name="Ivan")
print(ivan.say("hi"))  # напечатает "Ivan: hi"

sonya = Human("Sophia")
print(sonya.say("hello"))  # напечатает "Sophia: hello"

# Вызовем метод класса
ivan.get_species()  # => "H. sapiens"

# Изменим атрибут класса 
Human.species = "H. neanderthalensis"
ivan.get_species()  # => "H. neanderthalensis"
sonya.get_species()  # => "H. neanderthalensis"

# Вызовем статический метод
Human.grunt()  # => "*хрюк*"

# Изменим свойство
ivan.age = 42   # =>  напечатает "Ivan now 42 age old!"
...
# Модули питона - обычные файлы с кодом. Вы и сами можете написать такой.
# Интерпрератор считает модулями файлы с расширением .py и папки, в которых есть файл __init__.py , подгружаемый при импорте
...
# Если вы имеете питоновский скрипт <modulename>.py в той же директории, что и ваш вызываемый скрипт, <modulename>.py может быть подгружен вместо модуля <modulename> при импорте. Это не актуально для модулей стандартной библиотеки, но раборает для загружаемых модулей, таких как numpy. Старайтесь придумывать оригинальные названия своим скриптам во избежание коллизий 

# Ваш файл math.py в одной директории с исполняемым файлом заместит при импорте оригинальный math!

# Пути, по которым можно найти все доступные для импорта библиотеки python, хранятся sys.path 
# (как вы догадываетесь, сперва нужно подгрузить модуль sys) 


####################################################
# 7. Продвинутые функции языка 
####################################################
...
# Существует особый синтаксический сахар для генераторов списков
...
print(', '.join(eat_only_candy(['Soup', 'Salad', 'Tea', 'Chocolate'])))


# Исключения.
# Некондиционное поведение кода в хороших языках программирования обязано явным для отлаживающего код программиста способом выдавать ошибку. В Python эти ошибки реализуются с помощью механизма исключений (exceptions)
...
try:
    # Используейте "raise", что бы вызвать исключение
    raise IndexError("This is an index error")
except IndexError as e:
    pass  # Ключевое слово pass абсолютно ничего не делает. Всё равно что пустая строка. Применяется в роли заглушки блока для корректности блочного форматирования
...
# Оператор assert проверяет переданное в неё условие на истинность
#  и возвращает AssertionError , если это не так.
...
def arcsin(x):
    ...
    return x*(1 + x**2*(1/6 + x**2*3/40)) # первые три члена разложения арксинуса в ряд Тейлора

arcsin(1/2)*6 # 3.1390625 . Точность, конечно, не очень.
...
# Из этого следует, что хорошее знание синтаксиса необходимо но недостаточно для профессионализма, поскольку python здесь является вспомогательным инструметом управления чем-то, нередко, достаточно сложным в своей основе. В любом случае, его хорошее знание открывает следующие пути разивтия (названия условны):

# Математик. Занимается численным моделированием и обработкой данных.
# Обязательные технологии: библиотеки Numpy, Scipy, Jupiter, средства рисования графиков.

# Дата саентист. Подвид математика, занимающегося обработкой неких массивов данных методами статистики.
# Обязательные технологии: всё то же, что и математик + библиотека Pandas

# Нейросетевик. Машинное обучение и всё, что с ним связано
# Обязательные технологии: всё то же, что и математик + библиотека TensorFlow
...
# Обязательные технологии: другой язык программирования, в котором будет сделана основная реализация алгоритма. Например, C++. Может использовать python в качестве языка прототипирования и проверки идеи. Может соревниваться на codeforces.com и подобных ресурсах, обычно, без особых проблем, используя python в качестве основного языка. Крайне желательно знание Numpy, Scipy , что бы не терять в качестве при работе с массивами.
...
# Если вы зайдете в консоль, и введете import this ,
# вы увидите Дзен Питона - текст, отражающий общую философию языка.
